...
Device.prototype.enumeratePendingSpawn = function () {
  return this[$].enumeratePendingSpawn();
};

Device.prototype.enumeratePendingChildren = function () {
  return this[$].enumeratePendingChildren();
};

Device.prototype.spawn = function (program, options) {
  var pendingOptions = Object.assign({}, options || {});

  var argv = consumeOption('argv');
  ...
  var envp = consumeOption('envp');
  var env = consumeOption('env');
  var cwd = consumeOption('cwd');
  var stdio = consumeOption('stdio');
  var aux = pendingOptions;

  return this[$].spawn(program, argv, envp, env, cwd, stdio, aux);

  function consumeOption(name) {
    var value = pendingOptions[name];
    if (value === undefined)
      return null;
...
Device.prototype.injectLibraryFile = function (target, path, entrypoint, data) {
  return this[getPid](target).then(function (pid) {
    return this[$].injectLibraryFile(pid, path, entrypoint, data);
  }.bind(this));
};

Device.prototype.injectLibraryBlob = function (target, blob, entrypoint, data) {
  return this[getPid](target).then(function (pid) {
    return this[$].injectLibraryBlob(pid, blob, entrypoint, data);
  }.bind(this));
};
